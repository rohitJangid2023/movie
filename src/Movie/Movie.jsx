...
function Movie() {
    ...
    const fatchMovie = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=d8d56359455a8c1f58621b1cc4c24eef`);
    ...
    const MovieTrailer = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/movie/${id}/videos?api_key=d8d56359455a8c1f58621b1cc4c24eef`);
    ...
    const MovieCast = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=d8d56359455a8c1f58621b1cc4c24eef`);
            const data = await response.json();
            setCast(data.cast)
        } catch (error) {
            console.error('Error fetching data', error);
    ...
    return (
        ...
        <div>
            <Card className='min-h-[100vh] max-h-screen bg-black relative ' >
                <Card.Img className='max-h-screen !rounded-none' src={`https://image.tmdb.org/t/p/original${movieDetail.backdrop_path}`} />
                ...
                <Card.ImgOverlay className='!top-[unset] text-white max-w-1/2' >
                    ...
                    <Container className="anim">
                        ...
                        <Card.Body className='!px-0 '>
                            ...
                            >
                                <Modal.Body>
                                    <iframe className='w-full h-full' src={`https://www.youtube.com/embed/${trailer.key}`} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
